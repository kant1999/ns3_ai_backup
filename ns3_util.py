...
def build_ns3(path):
    ...
    for line in proc.stdout:
        if "cd build; cmake --build . -j 3 ; cd .." in line:
            ok = True
            break
    return ok
...
def run_single_ns3(path, pname, setting=None, env=None, show_output=False, build=True):
    ...
    if not setting:
        ...
        cmd = "./ns3 run '{}{}' 2>test.log".format(pname, get_setting(setting))
    if setting['RngRun']:
        if setting['RngRun']==7:
            print("runing cw7")
            n=setting['epoch']
            outstring='senario2_'+str(n)+'.log'
            cmd = ("./ns3 run '{}{}' 2>"+outstring).format(pname, get_setting(setting))
        if setting['RngRun']==5:
            print("runing scenario2 with remain_action")
            n=setting['epoch']
            outstring=str(setting['output_log_name'])+'senario2_remain_'+str(n)+'.log'
            cmd = ("./ns3 run '{}{}' 2>"+outstring).format(pname, get_setting(setting))
