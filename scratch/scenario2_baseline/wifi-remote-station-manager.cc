...
void
WifiRemoteStationManager::ScheduleNextStateRead (uint16_t id,double maxThroughput,double cur)
{
    NS_LOG_FUNCTION(this);
}
...
WifiRemoteStationManager::GetDataTxVector(const WifiMacHeader& header, uint16_t allowedWidth)
{
    ...
    Mac48Address address = header.GetAddr1();        //接收地址
    ...
    {   //std::cerr << " GetDataTxVector事件  data发生 " <<  std::endl;
