 ...
 //  acWifi.SetRemoteStationManager("ns3::AarfWifiManager",
  ...
  //********************************************senario_8***************************************************
   ...
   if( interference==1)
   {
        ...
        double power_interface=35;//设置干扰功率dbm
       
        double pathloss=10*3*log10(100)+46.67;//100米的路径损耗
        double power_dbm=power_interface-pathloss;//AP处接收到的干扰功率dbm
        double w=pow(10,power_interface/10)/1000;//干扰源处的发射功率W
        std::cout<<"干扰功率"<<power_dbm<<"dbm"<<std::endl;
        *wgPsd = w/ 80e6; // PSD spread across 20 MHz
                               ...
                               << " integrated power: " << Integral(*(GetPointer(wgPsd))));
        {
            ...
            waveformGeneratorHelper.SetPhyAttribute("Period", TimeValue(Seconds(0.01)));//干扰周期
            waveformGeneratorHelper.SetPhyAttribute("DutyCycle", DoubleValue(0.1));//占空比
